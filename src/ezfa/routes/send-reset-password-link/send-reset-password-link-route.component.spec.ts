...
describe('SendResetPasswordLinkRouteComponent angular sanity check', () => {
  ...
  describe('ngOnInit', () => {
    it('should load the user, then set the fg value if the user is present', fakeAsync(() => {
      ...
      component.ngOnInit();
      tick();
      expect(component.fg.get('email').value).toBe(HELPER.MOCK_USER.email);
    }));
  ...
  describe('submit()', () => {
    ...
    beforeEach(() => {
      ...
      spySend = spyOn(component.service.auth, 'sendPasswordResetEmail').and.callThrough();
    ...
    it('should handle success', fakeAsync(() => {
      ...
      tick();
      expect(component.service.auth.sendPasswordResetEmail).toHaveBeenCalledWith('foo@bar.com');
    ...
    it('should handled sfa/no-password', fakeAsync(() => {
      ...
      expect(component.ensureUserExistsWithPassword).toHaveBeenCalledWith('foo@bar.com');
      tick();
      expect(component.service.auth.sendPasswordResetEmail).not.toHaveBeenCalled();
      expect(component.submitting).toBe(false);
  ...
  describe('ensureUserExistsWithPassword(email: string)', () => {
    ...
    let result;
    beforeEach(() => {
      spyFetch = spyOn(component.service.auth, 'fetchProvidersForEmail').and.callThrough();
      result = undefined;
    });
